...
func NewRelay(name string, store Store, sender eventbus.Sender, opts ...Option) (*Relay, error) {
	...
	if _, canRelease := store.(relay.LeaderLockReleaser); hasLeaderStore && !canRelease {
		...
		options.Logger.Info("stream relay: store has no relay.LeaderLockReleaser capability; on shutdown the leader lease expires by TTL instead of releasing immediately",
			"relay", name)
	}
