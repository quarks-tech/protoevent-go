...
> (parks poison DecodeErrors ONLY; send failures always stop the lane), PBRT() -> Checkpoint(),
> ClusterTime/clusterTime -> CommitTime/commitTime in the neutral stream contracts (the mongodb
> store keeps clusterTime internally - it is the wire term), ErrStreamInvalidated -> ErrInvalidated,
> StreamStore -> Store (earlier), Observer is a struct of nil-able callbacks (OnDrained/OnError/
> OnSequenced/OnSwept/OnLeadership), LeaderElector moved to internal/leader (not public API),
> resume token is string (not []byte). See pkg/transport/outbox/README.md for current API.
Date: 2026-07-07
Owner: @filenko
...
pkg/transport/outbox/relay/          # shared primitives — no Relay type
    Observer, ErrorHandler, LeaderStore, LeaderElector helper (error policy: stop-the-lane / park)
pkg/transport/outbox/relay/sequence/ # sequence.Relay, Store, SequencerStore, RetentionStore   (TiDB)
pkg/transport/outbox/relay/stream/   # stream.Relay, Store, Stream, Event                      (Mongo)
```

- **`relay` (shared):** `Observer`, `ErrorHandler`, `LeaderStore` (both backends use a
  `relay_lock`-style lease), a `LeaderElector` helper (acquire / renew / graceful release), and the
  error policy (stop-the-lane default vs park-and-continue via the `ErrorHandler`). These are
  identical concerns.
- **`relay/sequence`:** the TiDB runtime and its `Store` / `SequencerStore` / `RetentionStore`
  (int64 `seq`). See the companion doc.
- **`relay/stream`:** the Mongo runtime and its `Store` (opaque resume token, `Watch`). This
  doc.

> This restructure renames the TiDB design's `relay.Relay` → `sequence.Relay`, `relay.Store` →
> `sequence.Store`, etc., moving `Observer` / `ErrorHandler` / `LeaderStore` / error policy up into
> `relay`. The TiDB implementation plan's package paths must be patched accordingly (it is not yet
> executed).
...
type outboxDoc struct {
    ID         string    `bson:"_id"`          // message ID (must be non-empty — CreateOutboxMessage
                                               // rejects "" up front, since an empty _id would only
                                               // fail on the SECOND publish with a far-from-cause
                                               // duplicate-key error; natural insert dedup)
    Metadata   []byte    `bson:"metadata"`     // CloudEvents metadata as JSON bytes (JSON caveat:
                                               // numeric extension values round-trip as float64)
    Data       []byte    `bson:"data"`
...
**Indexes:**
- `outbox`: implicit unique `_id` (message ID → natural insert dedup); **TTL index on
  `create_time`** with `expireAfterSeconds` ≥ the oplog window (§7) — default 7 days, configurable
  via `mongodb.WithRetention(d)`. MongoDB refuses to re-create an existing TTL index with a
  different `expireAfterSeconds` (IndexOptionsConflict): changing retention on an existing
  collection needs a `collMod` on the index, and `EnsureIndexes` surfaces that error with a hint.
  No `outbox_completed`, no delete-after-send.
...
loop:
  ...
  open/continue change stream:
      pipeline = [{$match: {operationType: {$in: ["insert", "invalidate"]}}}]   # §6a
      resumeAfter = stored token        (or omitted for a new group → stream opens at "now" — §6b)
      maxAwaitTimeMS = drain window     (DrainWindow, passed to Store.Watch as maxAwait)
  for each event in this window:
      on run-ctx cancellation → shutdown: STOP-THE-LANE, never the error handler
      msg = decode(event.fullDocument)
      on decode failure (*DecodeError), error handler set → park + resume past it
      on decode failure, NO error handler → STOP-THE-LANE at the poison event
      Sender.Send(msg)                  # per-event
      on send failure → STOP-THE-LANE, ALWAYS (see below; the ErrorHandler parks
          poison events only — a send failure is downstream trouble and must be
          retried in place, or a broker outage would bulk-divert the healthy
          backlog to the DLQ while advancing the persisted position past it)
  ...
  on invalidate event → Next returns ErrStreamInvalidated → fatal: log + ObserveError + stop (§6d)
  on ChangeStreamHistoryLost → Next returns ErrHistoryLost → break-glass DR (§7)
```
...
(reopen resumes from the prior token). A brand-new group has no prior token, so a fresh `Watch`
with `token == ""` immediately persists the stream's initial PBRT as the resume baseline, before
any draining — a first-window failure then still has a position preceding the failed event to
reopen from, instead of restarting at a new "now" and silently skipping it. Park-and-continue, by contrast, keeps the same cursor open
...
- **(a) What to watch:** pipeline
  `[{$match: {operationType: {$in: ["insert", "invalidate"]}}}]`. The collection is
  insert-only in normal operation, but TTL pruning emits `delete` change events — the match filter
  excludes them so the relay never tries to "deliver" a deletion. `invalidate` MUST pass the
  filter too: a `$match` on `insert` alone silently drops the collection-dropped/renamed
  invalidate event, leaving the stream reporting empty windows forever instead of surfacing the
  fatal error (§6d). Insert change events already carry
...
- **(b) New-consumer start position (v1: "now" only):** a new consumer group (no stored token)
  starts at **"now"** (no `resumeAfter` — the stream opens at the current cluster time) — it
  subscribes to future
...
- **(c) Token persistence cadence:** batch — persist once per drain window (`WithDrainWindow`,
  ...
  persists the lagging last-processed token, never the fresh PBRT). `SaveToken` is **monotone in
  clusterTime**: a save carrying an older clusterTime than the stored row is a no-op, so a stale
  leader finishing a slow window cannot rewind a newer position (§8.2 hardening note).
- **(d) Invalidate events:** if `outbox` is dropped/renamed the stream emits `invalidate` and
  closes. The store surfaces it as the sentinel `stream.ErrStreamInvalidated` returned from
  `Stream.Next` (mirroring `ErrHistoryLost`). Treated as a **fatal** relay error — log, report via
  `Observer.ObserveError`, stop the relay. Do not silently reopen from "now" (that would skip
  history). Surfaces a misconfiguration.
...
**longer than the oplog window**, the stream reopen fails with `ChangeStreamHistoryLost` (surfaced
as the sentinel `stream.ErrHistoryLost`); token *and* operation-time resume are both dead. Recovery must then come from the durable `outbox` collection —
...
// decided 2026-07-14; was an interface + a sequence-side superset interface):
// users set only the signals they care about, future signals are additive
// rather than breaking, and one type serves both runtimes.
type Observer struct {
    // OnDrained's count is successfully sent messages ONLY — a message
    // parked via an ErrorHandler is reported through OnError and not
    // counted here.
    OnDrained   func(name string, count int, oldestAge time.Duration, more bool)
    OnError     func(name string, err error)
    OnSequenced func(name string, count int) // fired by the sequence runtime only
}
...
// after which the relay advances past it. Send failures are never routed
// here — the lane always stops and retries in place (order and delivery
// preserved). Shutdown cancellation is never routed here either — a canceled
// run context stops the lane instead of parking healthy messages.
type ErrorHandler func(ctx context.Context, msg *outbox.Message, err error)

// NopObserver() returns the discard-all default observer.
...
// Error logging is per-runtime: WithLogger(*log/slog.Logger) on each Options set.
```
...
// at that edge; it is never the stored or Store-crossing representation.
...
type Store interface {
    ...
    // for the consumer group. Called once per drain window (§6c). Implementations
    // MUST be monotone in clusterTime: a save carrying an older clusterTime than
    // the stored row must not rewind it. persisted=false reports a save the
    // monotone guard skipped as stale — the relay then leaves its local
    // committed-position trackers (cliff-age reporting) untouched.
    ...
    // replay-from-beginning). maxAwait is the longest a single Next call may block
    // server-side waiting for events (maxAwaitTimeMS) — the relay passes its
    // DrainWindow, making it the single latency knob. The returned Stream yields
    // decoded messages plus their resume token + clusterTime, and exposes the
    // PBRT for empty windows.
    Watch(ctx context.Context, token string, maxAwait time.Duration) (Stream, error)
}

// Stream is a live change-stream cursor. Next blocks up to maxAwait (as passed
// to Watch) and returns (nil, ok=false, nil) on window timeout with no event;
...
// On error Next returns (nil, false, err), classified as:
//   - ErrStreamInvalidated: collection drop/rename (§6d) → fatal, the relay stops;
//   - ErrHistoryLost: resume token fell off the oplog (§7) → fatal, break-glass;
//   - *DecodeError: THIS event's payload failed to decode; carries the event's
//     resume position, so a relay with an ErrorHandler parks it and resumes past
//     it — without one the lane stops at the poison event (at-least-once,
//     order preserved);
//   - anything else is transient → the relay reopens the stream.
...
type Event struct {
    ...
    // no Invalidate flag: invalidation surfaces as the ErrStreamInvalidated
    // sentinel from Next (§6d), mirroring ErrHistoryLost
}

// Relay tails the outbox change stream for one consumer group and forwards to a
// Sender. Not safe for concurrent use: call Run from a single goroutine.
type Relay struct { /* name, store Store, sender eventbus.Sender, shared relay opts */ }

// NewRelay validates the configuration: non-empty name (it keys the token row
// and the default leader lock), DrainWindow/LeaseTTL/TokenBatchSize strictly
// positive, DrainWindow < LeaseTTL/2.
...
Options mirror the shared set where they apply: `WithLeaseTTL` (default 15s), `WithOpTimeout`
(default 30s — the bound on every individual store call, a SEPARATE knob from the lease so that
shortening failover cannot silently tighten every store deadline; must exceed `DrainWindow`, which
is the stream's server-side `maxAwaitTime`), `WithLeaderLockName`,
`WithObserver`, `WithLogger(*log/slog.Logger)`, `WithErrorHandler` (parks poison DecodeErrors
only; send failures always stop the lane), `WithDrainWindow(d)`, `WithTokenBatchSize(n)`. No `SequenceBatchSize`, no
`RetentionStore` (the store's TTL index handles pruning — `mongodb.WithRetention(d)`, default 7d),
and **no start-position option in v1** (always "now"; replay is the deferred backfill feature, §7).

`WithDrainWindow(d)` is the **single latency knob**: the relay passes it to `Store.Watch` as
`maxAwait`, where it becomes the change stream's `maxAwaitTimeMS` (default **1s**) — there is no
separate store-side await knob to keep in sync. It is a **separate
...
latency < LeaseTTL` to keep a single window inside one lease term. (The sequence runtime, by
contrast, renews the lease between full pages inside a pass, bounding its stale-leader overlap to
one page; the stream runtime's unit of overlap is one drain window, and `SaveToken`'s
clusterTime-monotone guard ensures a stale leader finishing a slow window cannot rewind the
committed resume position.)

### 8.3 `pkg/transport/outbox/mongodb` — MongoDB `stream.Store` + publish

- `Store` implementing `outbox.Store` (publish `InsertOne`; rejects an empty message ID up front —
  an empty `_id` would only fail on the SECOND publish with a far-from-cause duplicate-key error),
  `stream.Store` (`LoadToken` / `SaveToken` on `outbox_offsets`, `Watch` opening the change stream
  with the insert+invalidate `$match` pipeline and `SetMaxAwaitTime(maxAwait)` /
  `SetResumeAfter`; no `resumeAfter` → the stream opens at "now"), and `relay.LeaderStore`
  (the conditional-upsert `relay_lock` logic).
- `TryAcquireLeaderLock` evaluates lease expiry — and stamps the new deadline — on the **server
  clock** (`$$NOW` in an aggregation-pipeline upsert), never a client-side `time.Now()`: under
  clock skew between relay instances, a standby with a fast clock could otherwise steal a live
  lease (dual leader). Symmetric with the TiDB store, whose lock uses `NOW(6)`.
- `SaveToken` is **monotone in clusterTime** (§6c, §8.2): the update filter only matches a stored
  row whose `cluster_time` is ≤ the incoming one, and the resulting duplicate-key error on a stale
  save is swallowed as a no-op.
- `Watch` decodes each change event's `fullDocument` into `outbox.Message` (JSON-unmarshal the
  ...
  (resume token) + `clusterTime`. A payload that fails to decode surfaces as a `*stream.DecodeError`
  carrying that event's resume position (§8.2).
- `EnsureIndexes` creates the `create_time` TTL index (default 7d; `mongodb.WithRetention(d)`),
  idempotent for an unchanged retention; a changed retention surfaces the server's
  IndexOptionsConflict with a `collMod` hint (§4).
